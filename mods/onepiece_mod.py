        ...
        *,
        ...
        if success_list:
            pirate_messages = [
                ...
                f"Trafalgar Law's ROOM has temporarily removed {humanize_list([f'`{u}`' for u in success_list])}'s ability to communicate {time_str}!",
                f"Blackbeard's Dark-Dark Fruit has swallowed {humanize_list([f'`{u}`' for u in success_list])}'s messages into a void {time_str}!",
                f"Doflamingo's strings have tied up {humanize_list([f'`{u}`' for u in success_list])}'s fingers, preventing {'them' if len(success_list) > 1 else 'them'} from typing {time_str}!",
                f"Gecko Moria's shadows have stolen {humanize_list([f'`{u}`' for u in success_list])}'s ability to chat {time_str}!",
                f"Aokiji's Ice Age has frozen {humanize_list([f'`{u}`' for u in success_list])}'s keyboard solid {time_str}!",
                f"Kizaru moved at the speed of light and unplugged {humanize_list([f'`{u}`' for u in success_list])}'s internet connection {time_str}!",
                f"Kaido's Blast Breath melted {humanize_list([f'`{u}`' for u in success_list])}'s chat privileges {time_str}!",
                f"Marco the Phoenix's blue flames have temporarily incinerated {humanize_list([f'`{u}`' for u in success_list])}'s chat access {time_str}!",
                f"Ivankov's hormones have mysteriously changed {humanize_list([f'`{u}`' for u in success_list])}'s voice, making it impossible to type {time_str}!",
                f"Bartolomeo's Barrier Fruit has cut off {humanize_list([f'`{u}`' for u in success_list])} from the chat {time_str}!",
                f"Enel's lightning struck {humanize_list([f'`{u}`' for u in success_list])}'s computer, causing a temporary blackout {time_str}!",
                f"Garp threw his Fist of Love, knocking {humanize_list([f'`{u}`' for u in success_list])} out of the chat {time_str}!",
                f"Mihawk's sword slash was so precise, it cut {humanize_list([f'`{u}`' for u in success_list])}'s chat connection {time_str}!",
                f"Magellan's Venom Demon has poisoned {humanize_list([f'`{u}`' for u in success_list])}'s chatting ability {time_str}!",
                f"Perona's Negative Hollow made {humanize_list([f'`{u}`' for u in success_list])} too depressed to type {time_str}!",
                f"Caesar Clown's Gastinet made the air around {humanize_list([f'`{u}`' for u in success_list])} unbreathable, forcing {'them' if len(success_list) > 1 else 'them'} to retreat from chat {time_str}!",
                f"Fujitora's gravity crushed {humanize_list([f'`{u}`' for u in success_list])}'s keyboard {time_str}, making typing impossible!",
                f"Sengoku's Buddha shockwave sent {humanize_list([f'`{u}`' for u in success_list])} flying out of the chat room {time_str}!",
                f"Borsalino's light speed kick launched {humanize_list([f'`{u}`' for u in success_list])} into a communication dead zone {time_str}!",
                f"Bonney's Age-Age Fruit regressed {humanize_list([f'`{u}`' for u in success_list])} to an age before they could type {time_str}!",
                f"Queen's Plague Rounds infected {humanize_list([f'`{u}`' for u in success_list])}'s chat privileges with silence {time_str}!",
                f"Shirahoshi accidentally called a Sea King, who ate {humanize_list([f'`{u}`' for u in success_list])}'s messages {time_str}!",
                f"Corazon's Silent-Silent Fruit has created a zone of silence around {humanize_list([f'`{u}`' for u in success_list])} {time_str}!",
                f"Hody Jones' Energy Steroids caused {humanize_list([f'`{u}`' for u in success_list])} to rage-quit the chat {time_str}!",
                f"Kuma's Paw-Paw Fruit has deflected all of {humanize_list([f'`{u}`' for u in success_list])}'s messages {time_str}!",
                f"Capone Bege trapped {humanize_list([f'`{u}`' for u in success_list])} inside his body fortress, cutting off their communication {time_str}!",
                f"Hawkins' Straw-Straw Fruit has predicted a period of silence for {humanize_list([f'`{u}`' for u in success_list])} {time_str}!",
                f"Pudding's Memory-Memory Fruit made {humanize_list([f'`{u}`' for u in success_list])} forget how to type {time_str}!",
                f"Tama's Kibi-Kibi Fruit accidentally tamed {humanize_list([f'`{u}`' for u in success_list])}'s keyboard, and now it won't work for anyone else {time_str}!",
                f"Smoker's Smoke-Smoke Fruit has obscured {humanize_list([f'`{u}`' for u in success_list])}'s messages {time_str}!",
                f"X Drake transformed into a dinosaur and accidentally stomped on {humanize_list([f'`{u}`' for u in success_list])}'s communication device {time_str}!",
                f"Carrot's Sulong form was too bright, temporarily blinding {humanize_list([f'`{u}`' for u in success_list])} {time_str}!",
                f"Giolla's Art-Art Fruit turned {humanize_list([f'`{u}`' for u in success_list])}'s messages into abstract art, making them unreadable {time_str}!",
                f"Kinemon's disguise for {humanize_list([f'`{u}`' for u in success_list])} was so perfect, nobody could recognize {'them' if len(success_list) > 1 else 'them'} in the chat {time_str}!",
                f"Boa Hancock's Love-Love Beam turned {humanize_list([f'`{u}`' for u in success_list])}'s keyboard to stone {time_str}!"
            ]
