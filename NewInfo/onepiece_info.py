...
class OnePieceInfo(commands.Cog):
    ...
    async def onepiece_info(self, ctx):
        """Shows One Piece themed information about the Grand Line Discord."""
        ...
        embed.add_field(inline=False, name='Bot Information', value='I (Sunny 🌞) am an instance of Red-DiscordBot. If you want a bot like me (because I\'m as SUPER as Franky!), you can create your own by following the [Red installation docs](https://docs.discord.red/en/stable/install_guides/index.html).')
        embed.add_field(inline=False, name='Useful Commands', value='Use `!credits` and `!findcog` to view the other sources used in Sunny.')
        embed.add_field(inline=False, name='Getting Sunny', value="You might be wondering how to get Sunny for your own server. Currently, Sunny is a private bot for this Grand Line, but if you want to set sail with a bot like Sunny, you'll need to contact our Shipwright (server admin). Or better yet, build your own Red instance and customize it to be as SUPER as you want!")
        embed.add_field(inline=False, name='System Information', value=(f"**Python Version: {python_version} 🐍\nDiscord.py Version: {dpy_version} 📚\nRed Version: {red_version} 🔴\nPing: {ping}ms 🏴‍☠️**"))
        embed.add_field(inline=False, name='Server Slots', value=f"**Currently in {guild_count} / {max_guilds} servers ⚓**")
